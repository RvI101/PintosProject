...
/* Create syncronization strcutures */
static struct list file_list;
static struct lock list_lock; // we use this lock for filesys operations as well
...
/* Check validity of requested address */
...
syscall_handler (struct intr_frame *f)
{
  ...
  int offset=4,status,no=*stackpointer; /* Offset is the size of an argument - 4 bytes */
  void* sp=f->esp;
  uint32_t p;
  char* args;
  unsigned size;
